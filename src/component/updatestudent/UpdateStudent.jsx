...
const UpdateStudent = ({ closeModal, id, onSuccess }) => {
    ...
    const validation = () => {
        ...
        if (!Formdata.name.trim()) {
            ...
            setErrors(prev => ({ ...prev, name: newErrors.student_name }));
        } else if (!/^(?!.*\d).+$/.test(Formdata.name.trim())) {

            newErrors.student_name = " enter a valid name without numbers";
            setErrors(prev => ({ ...prev, name: newErrors.student_name }));
        }
        if (!Formdata.student_dob || !dayjs(Formdata.student_dob).isValid()) {
            newErrors.student_dob = "Date of Birth is required";
            setErrors(prev => ({ ...prev, student_dob: newErrors.student_dob }));
        }
        if (!Formdata.student_email.trim()) {
            newErrors.student_email = "Email is required";
            setErrors(prev => ({ ...prev, student_email: newErrors.student_email }));
        ...
        if (!Formdata.student_mobile.trim()) {
            newErrors.student_mobile = "Mobile is required";
            setErrors(prev => ({ ...prev, student_mobile: newErrors.student_mobile }));
        } else if (!/^(?!.*[a-zA-Z]).+$/.test(Formdata.student_mobile.trim())) {

            newErrors.student_mobile = " enter a valid Phone number";
            setErrors(prev => ({ ...prev, student_mobile: newErrors.student_mobile }));
        } else if (Formdata.student_mobile.trim().length !== 10) {
        ...
        if (!Formdata.student_father.trim()) {
            ...
            setErrors(prev => ({ ...prev, student_father: newErrors.student_father }));
        } else if (!/^(?!.*\d).+$/.test(Formdata.student_father.trim())) {

            newErrors.student_father = " enter a valid Father name without numbers";
            setErrors(prev => ({ ...prev, student_father: newErrors.student_father }));
        }

        if (!Formdata.parent_number.trim()) {
            newErrors.parent_number = "Parent's Mobile is required";
            setErrors(prev => ({ ...prev, parent_number: newErrors.parent_number }));

        } else if (!/^(?!.*[a-zA-Z]).+$/.test(Formdata.parent_number.trim())) {

            newErrors.parent_number = " enter a valid Phone number";
            setErrors(prev => ({ ...prev, parent_number: newErrors.parent_number }));
        } else if (Formdata.parent_number.trim().length !== 10) {
            ...
            try {
                const payload = {
                    ...
                    student_dob: Formdata.student_dob
                        ? dayjs(Formdata.student_dob).format("YYYY-MM-DD")
                        : ""
                };
    ...
    const getUserById = async (id) => {
        try {
            ...
            setFormdata({
                ...
                student_mobile: userData.mobileNo || '',
                student_father: userData.fatherName || '',
                parent_number: userData.alterMobileNo || '',
                student_address: userData.address || '',
                student_bloodgroup: userData.blood || '',
                student_qualification: userData.qualification || '',
                student_course: userData.courseDetails?._id || '',
                student_batch: userData.batchDetails?._id || '',
                student_aadhar: userData.aadharURL || '',
            ...
            if (userData.aadharURL) {
                setFileName(userData.aadharURL.split('/').pop()); // "aadhar.pdf"
            }
            if (userData.certificateURL) {
                setOriginal(userData.certificateURL.split('/').pop()); // "original.pdf"
            }

            if (userData.courseDetails?._id) {
                await getBatchnameid(userData.courseDetails._id);
            }
        } catch (err) {
